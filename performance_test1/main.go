...
func main() {
	fmt.Println("====================== ChainMaker 性能压测工具 (确定性冲突) ======================")
	fmt.Println("合约: test_rwset_contract")
	fmt.Println("方法: test_rwset")
	fmt.Println("场景: 不同冲突率下的性能测试（确定性冲突模式）")
	fmt.Println("==========================================================================\n")
	...
	testConfigs := []PerfTestConfig{
		{ConflictRate: 0, TotalTxCount: 1000000, GoroutineCount: 100}, // 10% 冲突率
		// 可以添加更多场景
...
// generateTransactions 生成交易（确定性冲突模式）
// 冲突率计算：假设有 N 笔交易，冲突率为 R
// - 冲突交易数：conflictTxCount = N * R
// - 无冲突交易数：nonConflictTxCount = N * (1 - R)
// - 无冲突部分：每笔交易使用唯一的 key（key_0, key_1, key_2, ...）
// - 冲突部分：所有交易使用相同的 key（conflict_key）
func generateTransactions(config PerfTestConfig) []Transaction {
	...
	fmt.Printf("  - 冲突交易数: %d (所有交易访问相同的 key)\n", conflictTxCount)
	...
	// 生成冲突交易：所有交易使用相同的 key
	conflictKey := "conflict_key"
	for i := nonConflictTxCount; i < config.TotalTxCount; i++ {
		transactions[i] = Transaction{
			ReadKey:    conflictKey,
			ReadField:  "", // 空，使用合约默认值 "data"
