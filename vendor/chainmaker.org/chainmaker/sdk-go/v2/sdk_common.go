...
func (cc *ChainClient) sendContractRequestWithPayer(payload *common.Payload, endorsers []*common.EndorsementEntry,
	payer *common.EndorsementEntry, timeout int64, withSyncResult bool) (*common.TxResponse, error) {
	resp, err := cc.proposalRequestWithTimeoutAndPayer(payload, endorsers, payer, timeout)
	if err != nil {
		return resp, fmt.Errorf("send %s failed, %s", payload.TxType.String(), err.Error())
	}
